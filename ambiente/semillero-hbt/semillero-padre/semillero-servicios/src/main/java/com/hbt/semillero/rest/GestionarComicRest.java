...
public class GestionarComicRest {
	 ...
	 * 
	 * Metodo encargado de traer la informacion de los precios de un comic
	 * http://localhost:8085/semillero-servicios/rest/GestionarComic/consultarComicVendidos
	 *
	 * @return
	 */
	@GET
	@Path("/consultarComicVendidos")
	@Produces(MediaType.APPLICATION_JSON)
	public Response consultarComicVendidos() {
		List<ConsultaTotalpersonajesComicDTO> comicDTO = null;
		try {
			comicDTO = gestionarComicEJB.consultarComicVendidos();
			//ResultadoDTO resultadoDTO = new ResultadoDTO(Boolean.TRUE, "Listado todos los Comic exitosamente");
			return Response.status(Response.Status.CREATED).entity(comicDTO).build();	
	 ...
	 * http://localhost:8085/semillero-servicios/rest/GestionarComic/consultarTotalPersonajesComic
	 * */
	@GET
	@Path("/consultarTotalPersonajesComic")
	@Produces(MediaType.APPLICATION_JSON)
	public Response consultarTotalPersonajesComic(){
		List<ConsultaTotalpersonajesComicDTO> total = null;
			try {
				total = gestionarComicEJB.consultarPersonajesComic();
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response modificarComic(@QueryParam("nombre") String nombre, @QueryParam("id") Long id,ComicDTO comicDTO) {
		
		try {
			gestionarComicEJB.modificarComic(nombre, id, comicDTO);
